...
class Availability(models.Model):
    ...
    repeat_option = models.CharField(max_length=100, blank=True)  # e.g., "daily", "Mon,Wed,Fri"
    notes = models.TextField(blank=True)
    language = models.CharField(max_length=100, blank=True)
    grade_level = models.CharField(max_length=100, blank=True)

    instant_booking = models.BooleanField(default=True)
    rate_type = models.CharField(max_length=50, choices=[('fixed', 'Fixed'), ('per_hour', 'Per Hour'), ('per_booking', 'Per Booking')], default='per_booking')
    ...
    tags = models.JSONField(default=list, blank=True)  # for SQLite compatibility
